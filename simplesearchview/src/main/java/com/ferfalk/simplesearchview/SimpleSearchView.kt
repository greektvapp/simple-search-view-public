...
class SimpleSearchView @JvmOverloads constructor(creationContext: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : FrameLayout(creationContext, attrs, defStyleAttr) {
    ...
    private fun initTooltips() = with(binding) {
        TooltipCompat.setTooltipText(backButton, context.getString(R.string.action_back));
        TooltipCompat.setTooltipText(clearButton, context.getString(R.string.action_clear_search));
    }
    ...
    fun setShowBottomLine(bool: Boolean) = with(binding) {
        bottomLine.visibility = if (bool) VISIBLE else GONE
    }
